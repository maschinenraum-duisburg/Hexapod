...
const int LED_PINS[] = {8,9,10,11,12,13};            //LED port definitions
const int LEDS_COUNT = 10;           //Number of leds per leg
...
void process_gamepad()
{
  ...
  {
    if(gait_speed == 0){
      LED_Bar(1,10);     //use green LEDs for fast
      Serial.println("start gehalten gait speed = 0");
    } 
    else {
      LED_Bar(0,10);                    //use red LEDs for slow
      Serial.println("start gehalten gait speed !=0");
    }
  }
  ...
  if(LED_color == 0)
  {
    for(int n=0;n<6;n++){
      for(int i=0;i<LED_count;i++){
        LEDS_LEG[n].setPixelColor(i, LEDS_LEG[n].Color(255,0,0)); // Moderately bright green color.
        LEDS_LEG[n].show(); // This sends the updated pixel color to the hardware.
      }
      for(int i=LED_count;i<10;i++){
        LEDS_LEG[n].setPixelColor(i, LEDS_LEG[n].Color(0,0,0)); // Moderately bright green color.
        LEDS_LEG[n].show(); // This sends the updated pixel color to the hardware.
      }
    }
  }
...
for(int n=0;n<6;n++){
      for(int i=0;i<LED_count;i++){
        LEDS_LEG[n].setPixelColor(i, LEDS_LEG[n].Color(0,255,0)); // Moderately bright green color.
        LEDS_LEG[n].show(); // This sends the updated pixel color to the hardware.
      }
      for(int i=LED_count;i<10;i++){
        LEDS_LEG[n].setPixelColor(i, LEDS_LEG[n].Color(0,0,0)); // Moderately bright green color.
